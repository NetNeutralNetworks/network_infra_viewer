...
class ObjectTypeView(BaseView):
    ...
    def list_object_types(self):
        ...
        q = f"""
            MATCH (d:Device)
            RETURN distinct d.object_type, avg(d.security_score), avg(d.reliability_score), avg(d.management_score), count(d)
        """
        lines = []
        for vrf in execute_query(q):
            object_type, security, reliability, management, count = vrf
            security, reliability, management = render_config_scores(security, reliability, management)
            lines.append(f"<a href='/objecttypeview/show/?type={object_type}'>{object_type}</a></td><td>{object_type}</td><td>{count}</td><td>{security}</td><td>{reliability}</td><td>{management}")
        ...
        help = """
            ...
            """
        return self.render_template('single_column_table.html', table_header="Locations </td><td>Object type</td><td>#devices</td><td>Average security</td><td>Average reliabilty</td><td>Average management", entries=lines, page_info=help, page_category='Network Components')
    ...
    def show_object_types(self):
        ...
        lines = []
        for vrf in execute_query(q):
            object_name, security, reliability, management, count = vrf
            security, reliability, management = render_config_scores(security, reliability, management)
            lines.append(f"<a href='/locationview/show/{object_name}'>{object_name}</a></td><td>{count}</td><td>{security}</td><td>{reliability}</td><td>{management}")
        ...
        help = """
            ...
            <p>This data is provided by the TOPdesk export.</p>
            """
        return self.render_template('single_column_table_with_map.html', table_header="Location</td><td>#devices</td><td>Average security</td><td>Average reliabilty</td><td>Average management", entries=lines, page_info=help, markers=markers, page_category='Network Components')
